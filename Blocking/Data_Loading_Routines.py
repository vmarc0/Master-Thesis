...
def load_adversarial(name='0.graph', basefolder = small_dataset):
...
def load_batched(Stop, root = small_dataset, N_BATCH=8):

    adj_data = []
    col_data = []
    
    #costruisce un batch mettendo in lista i grafi 
    for k, fname in enumerate(os.listdir(root)):
        adj, col = load_adversarial(fname, root)
        adj_data.append(adj)
        col_data.append(col)
        if k > Stop:
            break
